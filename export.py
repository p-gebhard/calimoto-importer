...
def get_routes(session_token: str, user_id: str, date_from: str | None, date_to: str | None, limit: int) -> list[dict]:
    data = _post("classes/tblRoutes", {
        **_auth(session_token),
        "_method": "GET",
        "where":   _date_where(user_id, date_from, date_to),
        "limit":   limit,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export Calimoto rides or planned routes")
    ...
    parser.add_argument("--limit", type=int, default=10_000,            help="Max number of entries")
    parser.add_argument("--type",  dest="export_type", choices=["rides", "planned"], default="rides",
    ...
    try:
        if args.export_type == "rides":
            entries = get_tracks(session_token, user_id, args.date_from, args.date_to, args.limit)
        else:
            entries = get_routes(session_token, user_id, args.date_from, args.date_to, args.limit)  # planned
    except RuntimeError as e:
        print(f"\nError: {e}")
        sys.exit(1)
    ...
    for i, entry in enumerate(entries, 1):
        ...
        if args.export_type != "planned":
            ...
            if "points" in files_data:
                ...
                (entry_dir / f"{date}_{oid}.gpx").write_text(_build_track_gpx(entry, pts, alts), encoding="utf-8")
        else:  # planned
            ...
            if url:
                try:
                    ...
                    (entry_dir / "points.json").write_text(
                        json.dumps({"points": pts}, ensure_ascii=False, indent=2), encoding="utf-8"
                    )
                    (entry_dir / f"{date}_{oid}.gpx").write_text(_build_route_gpx(entry, pts), encoding="utf-8")
                except Exception as e:
                    print(f"         ✗ points: {e}")
